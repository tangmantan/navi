...
- GitHub Pages auto-deploy workflow (`.github/workflows/deploy.yml`): builds and deploys on every push to `main`. The deploy path is auto-detected — domain root for `<user>.github.io` repositories, `/<repo-name>/` for project sites — and passed to the build via the new `VITE_BASE` environment variable consumed by `vite.config.ts`. The build also injects `VITE_API_URL` from repository variables (Actions Variables); when unset, the deployed site uses the local `config.json`.
